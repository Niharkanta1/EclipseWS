...
public class WindowUtils
{
    ...
    private static class W32WindowUtils extends NativeWindowUtils
    {
        ...
        public BufferedImage getWindowIcon(final WinDef.HWND hwnd) {
            ...
            long result = User32.INSTANCE.SendMessageTimeout(hwnd, 127, 1L, 0L, 2, 500, hIconNumber);
            if (result == 0L) {
                result = User32.INSTANCE.SendMessageTimeout(hwnd, 127, 0L, 0L, 2, 500, hIconNumber);
            }
            if (result == 0L) {
                result = User32.INSTANCE.SendMessageTimeout(hwnd, 127, 2L, 0L, 2, 500, hIconNumber);
            }
            if (result == 0L) {
                result = User32.INSTANCE.GetClassLongPtr(hwnd, -14);
                hIconNumber.getValue().setValue(result);
            }
            if (result == 0L) {
                result = User32.INSTANCE.GetClassLongPtr(hwnd, -34);
                hIconNumber.getValue().setValue(result);
            }
            if (result == 0L) {
                return null;
        ...
        public String getProcessFilePath(final WinDef.HWND hwnd) {
            ...
            final WinNT.HANDLE process = Kernel32.INSTANCE.OpenProcess(1040, false, pid.getValue());
