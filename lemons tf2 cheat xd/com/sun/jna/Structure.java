...
public abstract class Structure
{
    ...
    private List fieldOrder() {
        synchronized (Structure.fieldOrder) {
            List list = Structure.fieldOrder.get(this.getClass());
            if (list == null) {
                list = this.getFieldOrder();
                Structure.fieldOrder.put(this.getClass(), list);
            }
            return list;
        }
    }
    
    private List sort(final Collection c) {
    ...
    protected List getFields(final boolean force) {
        final List flist = this.getFieldList();
        final Set names = new HashSet();
        final Iterator i = flist.iterator();
        while (i.hasNext()) {
        ...
        if (fieldOrder.size() != flist.size() && flist.size() > 1) {
            if (force) {
                throw new Error("Structure.getFieldOrder() on " + this.getClass() + " does not provide enough names [" + fieldOrder.size() + "] (" + this.sort(fieldOrder) + ") to match declared fields [" + flist.size() + "] (" + this.sort(names) + ")");
            }
            return null;
        }
        else {
            final Set orderedNames = new HashSet(fieldOrder);
            if (!orderedNames.equals(names)) {
                throw new Error("Structure.getFieldOrder() on " + this.getClass() + " returns names (" + this.sort(fieldOrder) + ") which do not match declared field names (" + this.sort(names) + ")");
            }
    ...
    private void validateFields() {
        final List fields = this.getFieldList();
