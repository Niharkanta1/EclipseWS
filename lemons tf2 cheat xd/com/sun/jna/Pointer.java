...
public class Pointer
{
    ...
    public String[] getStringArray(final long offset, final int length, final String encoding) {
        ...
        if (length != -1) {
            ...
            int count = 0;
            while (count++ < length) {
                final String s = (p == null) ? null : ((encoding == "--WIDE-STRING--") ? p.getWideString(0L) : p.getString(0L, encoding));
                strings.add(s);
                if (count < length) {
                    addOffset += Pointer.SIZE;
            ...
            while ((p = this.getPointer(offset + addOffset)) != null) {
                final String s2 = (p == null) ? null : ((encoding == "--WIDE-STRING--") ? p.getWideString(0L) : p.getString(0L, encoding));
                strings.add(s2);
                addOffset += Pointer.SIZE;
            }
        }
    ...
    public void setWideString(final long offset, final String value) {
        Native.setWideString(this.peer + offset, value);
    }
    ...
    public String dump(final long offset, final int size) {
        ...
        final byte[] buf = this.getByteArray(offset, size);
