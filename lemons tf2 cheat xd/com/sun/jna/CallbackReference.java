...
class CallbackReference extends WeakReference
{
    ...
    private static Callback getCallback(final Class type, final Pointer p, final boolean direct) {
        ...
        synchronized (CallbackReference.callbackMap) {
            Callback cb = null;
            final Reference ref = CallbackReference.pointerCallbackMap.get(p);
            if (ref == null) {
                final int ctype = AltCallingConvention.class.isAssignableFrom(type) ? 63 : 0;
                final Map foptions = new HashMap(Native.getLibraryOptions(type));
    ...
    private CallbackReference(final Callback callback, final int callingConvention, boolean direct) {
        ...
        if (direct) {
            ...
            final long peer = Native.createNativeCallback(callback, this.method, nativeParamTypes, returnType, callingConvention, flags, encoding);
            ...
            final long peer = Native.createNativeCallback(this.proxy, CallbackReference.PROXY_CALLBACK_METHOD, nativeParamTypes, returnType, callingConvention, flags, encoding);
            this.cbstruct = ((peer != 0L) ? new Pointer(peer) : null);
        }
    }
    ...
    static void disposeAll() {
        final Iterator i = CallbackReference.allocatedMemory.keySet().iterator();
        while (i.hasNext()) {
            i.next().dispose();
        }
    }
    ...
    private static Pointer getFunctionPointer(final Callback cb, final boolean direct) {
        ...
        final int callingConvention = (cb instanceof AltCallingConvention) ? 63 : 0;
        final Map map = direct ? CallbackReference.directCallbackMap : CallbackReference.callbackMap;
        synchronized (CallbackReference.callbackMap) {
    ...
    private static class NativeFunctionHandler implements InvocationHandler
    {
        ...
        public NativeFunctionHandler(final Pointer address, final int callingConvention, final Map options) {
            final String encoding = options.get("string-encoding");
